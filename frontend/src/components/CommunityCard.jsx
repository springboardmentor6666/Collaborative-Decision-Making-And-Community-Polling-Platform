...
export default function CommunityCard({ community }) {
  ...
  const categoryName = community.categoryName || community.category?.name;
    ...
    >
      <div>
        <div className="mb-2.5 flex flex-wrap items-center gap-1.5">
          ...
          {categoryName && (
            <CategoryBadge name={categoryName} size="xs" />
          )}
        </div>
        ...
        {community.description && (
          <p className="mb-3 line-clamp-2 text-sm text-muted break-words [overflow-wrap:anywhere]">{community.description}</p>
        )}
      </div>
