        ...
        >
            <main className="mx-auto flex w-full max-w-5xl flex-col gap-5 p-3 sm:p-4 md:p-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <Link
                        to="/admin/analytics"
                        className="flex items-center gap-1.5 text-xs font-semibold text-[var(--app-secondary-text)] transition hover:text-violet-400"
                    >
                        <ArrowLeft size={14} />
                        Back to analytics
                    </Link>
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="flex items-center gap-1.5 rounded-lg border border-[var(--app-border)] px-3 py-1.5 text-xs font-semibold text-[var(--app-text)] transition hover:border-violet-400/50 hover:text-violet-400 disabled:opacity-50"
                    >
                        <RefreshCw
                            size={14}
                            className={
                ...
                <section className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-card)] p-4 shadow-sm sm:p-5">
                                ...
                                <button
                                    ...
                                    className={`rounded-full px-3 py-1.5 text-[11px] font-semibold transition ${
                                        type === option.value
                                            ...
                                            : "border border-[var(--app-border)] text-[var(--app-secondary-text)] hover:border-violet-400/50 hover:text-violet-400"
                                    }`}
                                >
                                    {option.label}
                                </button>
                            ...
                            <div className="space-y-1">
                                {items.map(
                                    (event, index) => (
                                        ...
                                        >
                                            {/* Violet timeline dot */}
                                            <div className="relative z-10 mt-[7px] h-[11px] w-[11px] shrink-0 rounded-full border-2 border-[var(--app-card)] bg-violet-500 shadow-[0_0_8px_rgba(139,92,246,.5)]" />
                    ...
                    {totalPages > 1 && (
                        <div className="mt-5 flex flex-col items-center gap-3 border-t border-[var(--app-border)] pt-4 text-xs sm:flex-row sm:justify-between">
                            <button
                                type="button"
                                onClick={() =>
                                    setPage((p) =>
                                ...
                                className="w-full rounded-lg border border-[var(--app-border)] px-3 py-2 font-semibold text-[var(--app-text)] transition hover:border-violet-400/50 hover:text-violet-400 disabled:opacity-40 sm:w-auto"
                            >
                                ← Previous
                            </button>
