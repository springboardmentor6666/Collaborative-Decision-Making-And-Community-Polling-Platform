-- V10: Add Recent Activities Table, High-Performance Composite Indexes, and Initial Feed Seed (PostgreSQL 15+)
...
-- DELETE FROM activities WHERE created_at < NOW() - INTERVAL '180 days';
...
SELECT 
    ...
    CONCAT('{"decisionId":', d.id, ',"title":"', REPLACE(d.title, '"', '\"'), '"}')::jsonb,
...
SELECT 
    ...
    CONCAT('{"pollId":', v.poll_id, ',"decisionId":', d.id, '}')::jsonb,
    CASE WHEN d.visibility = 'PRIVATE' THEN 'PRIVATE' ELSE 'PUBLIC' END,
    v.voted_at
FROM votes v
...
SELECT 
    ...
    CONCAT('{"commentId":', c.id, ',"decisionId":', d.id, '}')::jsonb,
    CASE WHEN d.visibility = 'PRIVATE' THEN 'PRIVATE' ELSE 'PUBLIC' END,
    c.created_at
FROM comments c
...
SELECT 
    ...
    CONCAT('{"communityId":', cm.community_id, ',"role":"', cm.role, '"}')::jsonb,
    'PUBLIC',
    comm.created_at
