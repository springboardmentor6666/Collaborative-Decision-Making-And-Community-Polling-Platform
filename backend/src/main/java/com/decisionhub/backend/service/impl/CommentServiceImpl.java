...
public class CommentServiceImpl implements CommentService {
  private final CommentRepository comments; private final DecisionRepository decisions; private final CurrentUserService current; private final DecisionService decisionService; private final NotificationService notificationService;
  public CommentServiceImpl(CommentRepository comments, DecisionRepository decisions, CurrentUserService current, DecisionService decisionService, NotificationService notificationService) { this.comments=comments; this.decisions=decisions; this.current=current; this.decisionService=decisionService; this.notificationService=notificationService; }
  ...
  public CommentResponse add(Long decisionId, CommentRequest request) {
    Decision decision = findDecision(decisionId);
    decisionService.toResponse(decision);
    User user = current.get();
    Comment saved = comments.save(Comment.builder().comment(request.getContent().trim()).user(user).decision(decision).build());
    if (decision.getCreatedBy() != null && !decision.getCreatedBy().getId().equals(user.getId())) {
      notificationService.notifyUser(decision.getCreatedBy(), user.getName() + " commented on your decision \"" + decision.getTitle() + "\"");
    }
